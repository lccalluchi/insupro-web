      ...
      <CardContent>
        ...
        <div className="grid gap-4">
          {filteredRecetas.map((receta) => (
            <div
              key={receta.id}
              className="bg-background p-4 rounded-xl shadow-sm border border-border hover:shadow-md transition-all cursor-pointer"
              onClick={() => handleOpenReceta(receta)}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4 flex-1">
                  <div className="h-12 w-12 bg-primary/10 rounded-lg flex items-center justify-center text-primary text-xl flex-shrink-0">
                    <UtensilsCrossed size={24} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="font-semibold text-foreground">{receta.nombre}</h4>
                      <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">
                        {receta.categoria}
                      </span>
                    </div>
                    <p className="text-sm text-muted-foreground truncate">{receta.descripcion}</p>
